...
class SQLCine(context: Context):SQLiteOpenHelper(context,"examen",null,1) {
    ...
    fun consultarCineTodo(): ArrayList<ECine> {
        ...
        if(existeUsuario){
            ...
            Log.i("bdd","No se pudo consultar los usuarios.")
        }

        Log.i("bdd", "Consulta Satisfactoria")
    ...
    fun cossultarPelicula(id_cine: Int): ArrayList<EPelicula> {
        val scriptConsultarCasa = "SELECT * FROM PELICULA WHERE ID_CINE_A = ${id_cine}"
        val baseDatosLectura = readableDatabase
        ...
        //val Pelicula = EPelicula(0,0,"","",0.0,false)
        if(existePelicula){
            do{
